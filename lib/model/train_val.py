...
#    self.data_layer_val._perm = np.random.permutation(np.arange(len(self.data_layer_val._roidb)))   
    ...
    while iter < max_iters + 1:
      ...
      blob_large, blob_small = self.data_layer.forward()

      now = time.time()
      if iter == 1 or now - last_summary_time > cfg.TRAIN.SUMMARY_INTERVAL:
        # Compute the graph with summary
        rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, total_loss, summary = \
          self.net.train_step_with_summary(sess, blob_large, blob_small, train_op)
        self.writer.add_summary(summary, float(iter))
        # Also check the summary on the validation set
        blobs_val_large, blobs_val_small = self.data_layer_val.forward()
        summary_val = self.net.get_summary(sess, blobs_val_large, blobs_val_small)
        self.valwriter.add_summary(summary_val, float(iter))
        last_summary_time = now
      else:
        # Compute the graph without summary
        rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, total_loss = \
          self.net.train_step(sess, blob_large, blob_small, train_op)
      timer.toc()
